...
@profile
def test():
    book = OrderBook(Instrument.FUTURE, 0.01, 0.01)

    for i in range(1, 100000):
        side = Side.BUY if random.randint(0, 1) else Side.SELL
        order = Order(i, Instrument.FUTURE, Lifespan.GFD, side,
                      random.randint(95, 105), random.randint(1, 10))
        book.insert(time.monotonic(), order)
